...
Board has the following internal variables:
-turn: string of either "X" or "O" depending on who's turn it is
-pos1 through pos9: stores what is currently in that board spot. either "X" "O" or the number
corresponding to the board position
Board has the following functions:
-print board which prints the current board
-win?: evaluates if the board is a winning position. Returns 1 for X wins and -1 for O wins
-Move: takes a board position as an argument. Changes the board position variable
to reflect the move as well as the turn variable

Then have a Running variable that is true until the game is over

While Running:
-get move
-call board.move() on the input
-if illegal move then get new input
-print the board
-check win?: if win evaluates to 1 say "X wins" and set Running to False. If -1, same for O. if 0, then keep going.
-store a variable move_num that keeps track of how many moves have been made. If it reaches 9 and win? returns 0 then
the game is a draw
Board Structure:
...
# data tree implementation NOT USED
# class TreeNode:
...
while gamerunning:
    ...
    # loop for two players playing against each other FIXME: add a way to do this too?
    # playermove = int(input("Please enter your move\n"))
    # if gameboard.move(playermove) == "illegal move":
    #     print("illegal move")
    # else:
...
# testing
# myboard = Board("X", "1", "2", "3", "4", "5", "6", "7", "8", "9")
# myboard.printboard()
# myboard.move(1)
# myboard.move(4)
# myboard.move(2)
